...
def run_physical_policy(policy: str, data: dict, input_sha256: str, log: RunLog) -> dict:
    ...
    attempt = f"{policy_slug(policy)}_attempt_{time.strftime('%Y%m%dT%H%M%S')}_{time.monotonic_ns()}"
    ...
    try:
        ...
        origin_ns = time.monotonic_ns() + 1_000_000_000
        ...
        while True:
            ...
            while idle_workers and waiting:
                ...
                enqueue_due(time.monotonic_ns())
                worker = min(idle_workers)
                decision_clock_ns = time.monotonic_ns()
                ...
                send_return_ns = time.monotonic_ns()
            ...
            timeout = (
                max(0.0, (next_wake_ns - time.monotonic_ns()) / 1e9)
            )
