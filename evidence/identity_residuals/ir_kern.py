"""INSTRUMENTED COPY of ../guard_variants/guardkern.py, sha256
e8b8578d5b53874440d2f37f285bc422c127512595b544fe5427b5a3acdd1c58 (the hash the v3.1
manifest in ../main_v3/out_main_v31.txt carries for it).

The scheduling logic is byte-for-byte the original.  The only edits, all additive, are
the four marked `# [ir]` below: `simulate` now also fills

    dord[j]   the position of j in the DISPATCH SEQUENCE (0-based, §1.1 of
              ../guard_theory/theory.md: dispatches form one sequence, several may share
              an instant, `j -< i` means dord[j] < dord[i]),
    dtime[j]  the clock instant at which j was dispatched (= its start time; a maximal
              run of equal dtime along the sequence is a PHASE),

and `run` hands them back on the result object.  Nothing else is touched: no branch, no
comparison and no state update is changed, so the waits this file produces are bit-for-bit
the waits `guardkern.run` produces.  `ir_validate.py` checks that claim, job for job, on
random instances, on a real 5,000-job slice and on the full 17.6 M-job traces.
...
One numba kernel covers every policy studied here; the rule is fixed by its parameters.
    ...
    The count channel exists only when theta > 0 AND Ncap > 0; `run()` refuses any
    other combination.  When it exists, a completed overtaker whose true service is
    <= theta charges 1 to the count channel instead of charging its work to the work
    channel; a longer one charges its work as usual.  When it does not exist (the
    default theta = 0, Ncap = 0) every completed overtaker charges its work, and
    theta is not read at all.  The rule "charge nothing for a short overtaker",
    which has no per-job bound (see NOT GUARANTEED below), is therefore not
    reachable: it would need theta > 0 with the count channel switched off, and
    `run()` raises on that.
...
NOT GUARANTEED.  Charging nothing for short overtakers (theta > 0 with the count
channel off) and age-blended ranking (argmin pred - alpha*age) admit no per-job bound;
brute.py builds the counterexamples.  The first of the two cannot be selected through
`run()`: theta > 0 requires Ncap > 0, so a short overtaker always costs something (one
unit of the count budget).  This is the D1 item of the second referee report --
previously the kernel silently fell back to the plain work channel (correct and
guaranteed, but not what this docstring described), so the docstring, not the code, was
what needed fixing; the check below makes the intent explicit.
"""
import numpy as np
...
def simulate(a, s, pred, k, mode, C0_us, gam_us, en, ed, theta_us, Ncap, Bmax_us, Mslots):
    ...
    dord = np.full(n, -1, np.int64)                                          # [ir]
    dtime = np.full(n, np.nan, np.float64)                                   # [ir]
...
def run(a, s, k, policy="fcfs", pred=None, B=0.0, eps=0.0, gam=0.0, theta=0.0,
    ...
    theta and Ncap are the two halves of one option and must be set together: theta
    is the size below which a completed overtaker is charged to the COUNT budget
    Ncap instead of to the work budget.  theta > 0 with Ncap = 0 would mean "short
    overtakers are free", which has no per-job guarantee, so it is refused rather
    than silently ignored; Ncap > 0 with theta = 0 would set up a count budget that
    nothing can ever charge.
    """
    if (theta > 0) != (Ncap > 0):
        raise ValueError(
            "theta and Ncap must be set together (got theta=%r, Ncap=%r): theta>0 "
            "with Ncap=0 is the unbounded 'free short overtakers' rule, and Ncap>0 "
            "with theta=0 is a count budget nothing can charge" % (theta, Ncap))
    if theta < 0 or Ncap < 0 or Bmax < 0 or gam < 0 or eps < 0:
        raise ValueError("gam, theta, Ncap, Bmax, eps must be >= 0 "
                         "(B < 0 is the documented 'work channel off' sentinel)")
    ...
    (r.w, r.cbud, r.n_disp, r.n_forced, r.qw_total, r.qw_forced, r.n_rebuild,
     r.max_span, r.err, r.dord, r.dtime) = simulate(                         # [ir]
