...
def main():
    ...
    d.loc[(d.part==3)&(d.queue==2),'L']=5400
    ...
    raw_usable=d.copy()
    before=len(d); lo,hi=np.array(OUTAGE)*DAY-origin
    ...
    filters.append(dict(filter='arrival or observed lifetime intersects original SWF outage [138d,153d)',before=before,removed=before-len(d),after=len(d)))
    audit['outage_seconds_after_cleaned_start']=[int(lo),int(hi)]
    audit['outage_clock_correction']='Previous audit subtracted the first cleaned submit before applying raw SWF log-days. Corrected before any validation/model fit.'
    ...
    for pool in ['all',*NOMINAL]:
        ...
        for queue in ['all',*sorted(p.queue.unique())]:
            ...
            rows.append(dict(pool=pool,queue=queue,L=int(s.L.max()),L_min=int(s.L.min()),**stats(s.wait),**tails(s.c),raw_top1=tails(s.run)['top1'],raw_top5=tails(s.run)['top5'],exceeds=int((s.run>s.L).sum())))
