...
\abstract{Programming-assignment judges, build farms and serverless platforms run jobs without preemption on a fixed pool of $k$ servers, under a per-job time limit $L$. Ordering the queue by predicted cost shortens most waits, but one wrong prediction can leave one job far behind. We ask what such a service can promise to every job when the predictions are arbitrary. We prove a pathwise identity: under any non-preemptive work-conserving $k$-server policy, a job's delay relative to first-come first-served (FCFS) equals the net work that overtook it divided by $k$, to within $(2-2/k)L$, and this constant is exact. A budget on overtaken work is therefore sufficient for a per-job guarantee and necessary up to an additive $O(kL)$; a count of overtakes is not. A wrapper that enforces the budget bounds every job's wait against FCFS for arbitrary arrivals, predictions and base policies, with the exact additive constant $(3-2/k)L$, and the operator sets its cap from a promise stated in seconds. We evaluate on logs from two judging platforms, a serverless trace, two continuous-integration pools and a compute farm, checking every simulated job against the bound. Under conservative outcome visibility, a \devnum{600}-second guard closes \devnum{0.429} of the FCFS-to-shortest-job-first tail-wait gap at the busiest development load, with interval \devnum{[0.357, 0.533]}. We also report where the guarantee is too coarse for the observed waits and where cost concentration offers little to gain.}
