...
% --- Journal logo on the title page ---------------------------------------
% In `submit' status mdpi.cls prints only the MDPI logo, at the right. The
% journal's own Word template (mathematics-template.dot, MDPI, 2025) prints
% the MDPI logo at the left and the Mathematics logo at the right, and that is
% the look the class itself uses once the status is `accept'. The patch below
% gives the submission build the same header. It rewrites one line of the
% title macro at load time; mdpi.cls is not edited. Definitions/
% mathematics-logo.png is the logo image taken from that Word template.
\makeatletter
\patchcmd{\@maketitle}%
...
900-second test class at its own limit; Table S23: Scope of the nearest per-job
...
remark, and the relaxations of the assumptions behind
