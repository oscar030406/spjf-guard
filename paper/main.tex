...
points and the nine per-shape grid winners; Table S6: The age-relative and queue-length
winners at three promises and three loads; Table S7: Deliberately corrupted scores,
unguarded and under the guard; Table S8: The residual of
Theorem~\ref{thm:identity} on all nine cells; Table S9: The single-server configuration;
Table S10: Additional policy definition for linear aging; Table S11: Conservative-score
comparison with the selected aging baseline; Table S12: Package development AUROC with
user-block intervals; Table S13: Ranking scores fitted on one feature set and run through
one simulator; Table S14: Trace-construction sensitivities; Table S15: Unguarded
policies, five overlays; Table S16: Candidates and feasible points after promise
assignment and the harm filter; Table S17: Capacity sweep with archived scores; Table
S18: Physical timed-work runs; Table S19: Open-loop against think-time-preserving replay;
Table S20: LPC-EGEE by walltime class, both partitions pooled; Table S21: Replay of the
900-second test class at its own limit; Table S22: Scope of the nearest per-job
...
of its extremes, Proposition~\ref{prop:consistency} with the two counterexamples to its
converse, and the relaxations of the assumptions behind
...
and the LPC-EGEE counterfactual; Section S7, additional theory, with
reservation-and-refund charging under enforced run-time caps; Section S8, the extremal
constructions, together with achievability and the maximal permission rule at one server;
and Section S9, the extended related work behind Section~\ref{sec:related}, every work
named there being cited in the manuscript as well.}
...
% Section S4.10, which also precedes it).
