...
extremal properties of first-come first-served for the unfinished-work vector
are classical~\cite{daley1987fcfs,kiefer1955queues}; what carries the
single-server argument to $k > 1$ is a two-sided bound between two arbitrary
policies rather than an ordering.

\begin{lemma}[Workload comparison]
\label{lem:gap}
Let $A$ and $B$ be non-preemptive work-conserving $k$-server policies run on the
same input. Then $|U_A(t) - U_B(t)| \le (k-1)L$ for every $t$, and the
inequality is strict: $|U_A(t) - U_B(t)| < (k-1)L$. The constant is the exact
supremum, approached but never attained.
\end{lemma}

The difference $D = U_A - U_B$ is continuous and piecewise linear, and it cannot
increase at any instant at which $U_A$ exceeds $(k-1)L$, since more than $k-1$
jobs are then present under $A$ and every server of $A$ is busy; tracking $D$
back to the last instant at which $U_A$ was below the threshold gives the bound.
...
Lemma~\ref{lem:gap} gives $|R^P_i - R^{\mathrm{FCFS}}_i| < (k-1)L$. Subtracting
...
The error term of Theorem~\ref{thm:identity} looks like an artefact of the
proof, and it is not, although neither search nor real input shows it. Exhaustive
...
Section~\ref{sec:experiments} the largest residual reaches \devnum{0.485} to
\devnum{0.521} of $2(k-1)L$, again about $(k-1)L$ (Table~\ref{tab:resid}). The
...
\begin{remark}[The two directions are not alike]
...
dispatch-sequence convention because $\mathrm{In}_i$ is the quantity a scheduler
can meter: the guard charges completed work, which lower-bounds
$\mathrm{In}_i$ but not its executed-work variant.
\end{remark}

\subsection{Necessity, and why positions are the wrong currency}

Theorem~\ref{thm:identity} also runs backwards. A policy that promises anything
...
$\delta = G - \mathrm{excess}_P[i] \ge 0$. Of the jobs one server starts inside
such a window, all but the last complete inside it, so their total service is at
most $\delta$ and the last adds at most $L$.
\end{proof}
...
\begin{remark}[Counting overtakes is neither sufficient nor necessary]
\label{rem:counts}
Bounded-overtaking mechanisms in the literature limit how many times a job may
be passed~\cite{grosof2021nudge,vanhoudt2022nudgek,grosof2022wcfs}. Without a
bound on service times one overtake makes the excess arbitrarily large: at
...
and the victim waits $X$ with an overtake count of $1$. Conversely a count bound
is not necessary: with $k = 1$, $L = 2M$, a victim of service $L$ arriving first
...
With services bounded by $L$ a count wrapper does give a guarantee, and how the
count is charged decides which one. Charge it when an overtaker is
...
at most $N$ overtakers are dispatched while $i$ waits, $\mathrm{In}_i \le NL$, and
Corollary~\ref{cor:sufficient} gives
\begin{equation}
  W_P[i] \;\le\; W_{\mathrm{FCFS}}[i] + (N + 2k - 2)\,L/k ,
  \label{eq:skip}
\end{equation}
which is the accounting and the formula used wherever this baseline appears
below. Inverting it, a promise $G$ admits $N = Gk/L - (2k-2)$ passes. Charging
...
\begin{align}
  ...
  (1 - \eta)\,W &\;\le\; W_{\mathrm{FCFS}}[i] + B_0/k + (3 - 2/k)L ,
  \label{eq:guardmult}
\end{align}
...
$kW - \eta k W \le k W_{\mathrm{FCFS}}[i] + B_0 + (3k-2)L$; divide by $k$.
\end{proof}
...
$k = 1$, $L = 1$, $B_0 = 0$ and $B_{\max} = \gamma$; a job that arrives behind a
single waiting job has budget $\gamma$ and can be made to wait exactly $\gamma$
longer than under first-come first-served, while the right-hand side of
\eqref{eq:guardmult} is $2/(1-\eta)$ whatever $\gamma$ is. The same happens for
...
rounds and then injects a sequence timed so that $\mathrm{over}[i]$ stops one
unit short of the budget twice in succession, letting the base policy fill every
server a third time before the guard fires. Appendix~\ref{app:tight2} gives both
...
on the same instance, which is what makes the composition tight rather than each
part separately, and the excess is real waiting time rather than bookkeeping:
...
\subsection{When the guard is invisible}
...
maximum $\Lambda(t) = \max\{C_j : a_j \le t\}$ is placed correctly. The upward
...
and $V = \sum_m v_m$, Theorem~\ref{thm:identity} holds with $k$ replaced by $V$
...
\item \emph{Setup times survive.} If a setup of length $\sigma$ is paid before
every service under both policies, absorb it into the job as
$C'_j = C_j + \sigma$; the model is unchanged and every result holds with
$L \mapsto L + \sigma$.
...
\label{prop:floor}
Let $P$ be any non-preemptive work-conserving policy that observes a job's
service time only at its completion. Then there are inputs on which some job has
$\mathrm{excess}_P[i] \ge L$. In particular no wrapper with a positive budget
can promise $G < L$, and at $k = 1$ the bound $B + L$ of
...
once. Work conservation permits $P$ to fill all $k$ servers with the
higher-ranked jobs, and $P$ cannot be dissuaded from doing so, having observed no
service time yet, so $i$ waits until the first completion and
$\mathrm{excess}_P[i] = L$; any wrapper has $\mathrm{over}[i](0) = 0$ and does
not fire at $t = 0$. At $k = 1$, \eqref{eq:guardin} gives
...
\label{thm:price}
Take $m$ jobs of service $L$ and $n$ jobs of service $s < L$, the long ones
...
\[
  ...
  \;\le\; \frac{kG + (3k-2)L}{n s} \;+\; \varepsilon ,
\]
with $\varepsilon = O\bigl((m+n)/(mn)\bigr) \to 0$ as $m, n$ grow.
\end{theorem}
...
than a weakness of this wrapper. At $k = 1$ the achievable fraction is exactly
$\min(1, G/(ns))$.
...
is small relative to the waits an operator cares about and empty when it is not.
Section~\ref{sec:exp_cross} makes that boundary quantitative on six workloads,
one of which falls outside it.
