...
lowered are in Supplementary Sections~S4.1 and~S8.1.
...
$k-1$ jobs of service $L$ in the victim's own phase. Supplementary Section~S8.2
gives both in full, with the exact values they attain. Non-attainment follows
from Lemma~\ref{lem:gap}.

The two directions do not have the same content: upward the extremal value is a statement
about delay, while downward half of it is the bookkeeping convention that charges an
overtaker dispatched in the victim's own phase, which executes nothing while the victim
waits. Supplementary Section~S4.3 gives the remark in full, with the same-phase shares,
the executed-work variant under which the upward supremum is unchanged and the best
downward witness falls to $(k-1)L$, and the two reasons we keep the dispatch-sequence
convention.
...
The proof inducts on dispatch epochs and is in Supplementary Section~S4.2,
...
base policy and an input with excess at least $L$, and Supplementary Section~S8.4 gives
the single-server family that approaches the bound $B+L$ without reaching it.
...
as its own proposition and checks exhaustively over \devnum{1{,}161} parameter
settings.
...
Supplementary Section~S8.6 proves these statements. Supplementary theory
