...
Consequently no promise $G < L$ can be made that holds for every base policy, or
for one prediction-ranked base policy under arbitrary predictions, which is the
generality in which Theorem~\ref{thm:guard} is stated; and at $k = 1$ the bound $B + L$ of
...
does not release $i$ at $t = 0$. Take $A$ to be a policy that prefers the $k$
higher-ranked jobs; a prediction-ranked policy does so as soon as $i$ carries the
largest prediction, work conservation permits it, and $A$ has observed no service
time that could tell it the $k+1$ jobs are alike. All $k$ servers are filled, so
...
whatever the base policy. For the budget of Algorithm~\ref{alg:guard} these cases
...
\]
with $\varepsilon = O\bigl((m+n)/(mn)\bigr) \to 0$ as $m, n$ grow; explicitly,
$\varepsilon\,mn/(m+n)$ stays above
$2(k-1)L\bigl(1 + (kG + (3k-2)L)/(n\sigma)\bigr)/(L-\sigma)$ and tends to
$2(k-1)L/(L-\sigma)$ (Appendix~\ref{app:price}).
\end{theorem}
