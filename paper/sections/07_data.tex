...
only on reimaging or quarantine. The released tables of the second judging platform have no
timestamp column, only a submission id, so job order inside a group and job sizes are real while
...
The platform that produced the main trace creates a separate container for each
student and runs their code inside it~\cite{codebench_home}. Its logs do not identify the waiting times
of a shared judging queue. We use recorded durations and reconstructed start-time
proxies as inputs to an assumed shared pool; its capacity, queue and service
...
queue is a standard deployment. A widely used contest judging system recommends ``one
...
graders and measures the queue wait~\cite{peveler2019sandbox}. Dedicated per-entity
capacity is also known to sit idle in workloads like this one, which two
measurements outside our own traces record~\cite{shahrad2020wild,verma2015borg}. The
second support is a measurement on the same demand. Replayed under the dedicated
...
servers. The third is that the simulator reproduces waits somebody else recorded:
refitting an effective capacity and a setup term on one chronological part of each
continuous-integration window returns the held-out mean wait, the per-hour profile and
the per-job ordering. Supplementary Section~S2.3 gives all three in full. It carries
what the two external measurements do not license, the six session reconstructions, the
five chronological splits, the sharpness check, the one quantile that does not survive
and the circularity of the original whole-window fit. The pooled side is simulated, so none
of this supports a claim that the platform ever queued.
...
whole weeks, which covers time and leaves the choice of terms not covered. A trace built
...
% (lgbm_lags, lgbm_cal_only, lgbm_lags_cal) and paired CV h=168 rows.
 The supporting trace shows the same shape
...
the first id have been read; on the supporting trace it is the \devnum{2014}
