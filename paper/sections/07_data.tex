...
(Supplementary Table~S3). Groups of runs by the same user that end in the same second are
...
design, the 99th percentile of containers alive at once is \devnum{1{,}636} and each
container executes code for \devnum{0.12\%} of the time it is alive, while a shared
pool holds the 99th percentile of deadline-window waits under \devnum{1} s with
\devnum{14} servers under FCFS and \devnum{13} under the predictor-ordered policy. The
third is that the simulator reproduces waits somebody else recorded: the CI logs carry
the queue wait of every run, and an effective capacity and a setup term refitted on one
chronological part of each window return the held-out mean wait, the per-hour profile
and the per-job ordering, with the 90th percentile the one quantile that does not
survive. Supplementary Section~S2 gives both in full, with the six session
reconstructions, the five chronological splits, the step-of-one sharpness check and the
...
pools, which motivates the empirical cost-concentration screen (Supplementary Table~S3, which also
records the provenance of the analysis bound $L$ on every trace). Supplementary
Section~S2 gives what the arrival and service fields of each trace are, which of them
are proxies, and how many jobs each analysis truncation touches.
