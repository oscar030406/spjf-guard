...
(Corollary~\ref*{M-cor:params} there, $B_{\max} = k(G-(3-2/k)L)$) is $B=0$, so the
...
\[
 ...
 =\Gamma_P(a_i)-\Gamma_{\mathrm{FCFS}}(a_i).
\]
Substitution into the two busy-period equalities of the main text (the displayed equality $k\,W_P[i] = R^P_i + \mathrm{In}_i - \mathrm{Out}_i - \beta^P_i$ in the proof of the identity), one for $P$
and one for FCFS, proves the formula. A same-phase overtaker enters both
$\mathrm{In}_i$ and $\beta^P_i$ with its full size and therefore cancels.
\end{proof}
...
$\beta^{\mathrm{FCFS}}_i\leq(k-1)L$. This is not a deployable rule under the
...
$a_i$, by $\Lambda_{k-1}(a_i)$. Drop $-\beta^P_i\leq0$. The at most $k-1$ FCFS
jobs still running when $i$ starts all have rank below $i$, so
$\beta^{\mathrm{FCFS}}_i\leq\Lambda^{<i}_{k-1}$.
\end{proof}
...
Section~\ref*{M-sec:limits} of the manuscript states that the obvious escape from the $\Omega(L)$ floor
...
that the service enforces, whereas Sections~\ref*{M-sec:scheduling} and~\ref*{M-sec:theory} of the manuscript assume a service
that learns a job's cost only at its completion. That is why this is a supplementary
subsection and not a
theorem of Section~\ref*{M-sec:theory}: admitting it into the main line would put two information models
side by side.
...
$G \ge L$, which meets the $L$-scale floor of Proposition~\ref*{M-prop:floor} of the manuscript with
equality, and for general $k$ the condition is $G \ge (2-1/k)L$ against
$G > (3-2/k)L$ for Algorithm~\ref*{M-alg:guard} of the manuscript, a difference of $(1-1/k)L$, which is
...
Section~\ref*{M-sec:scheduling} of the manuscript is unchanged.
