...
platform these logs do not identify a shared
judging queue \cite{codebench_dataset}. We use its logs as a demand trace: the
arrivals are reconstructed and execution durations are recorded; the server pool, the
...
waiting on the real platform. The physical timed-work experiment demonstrates an
executing queue and guard, but its selected overlay window, empty initial state and
requested sleep durations do not validate transport of submitted-program costs. Its
rule-conformance checks coexist with a worse guarded p99 and large per-job deviations
from the ideal simulator (Supplementary Section~S6). The
...
and its direction is not identified. Faster replies can induce more submissions,
change their costs and alter which jobs enter a deadline-window quantile. The results
are therefore conditional on fixed offered work, rather than lower bounds on
closed-loop benefit. Shmueli and Feitelson distinguish increased productivity from
improvement in conventional waiting metrics \cite{shmueli2009simulation}; the exact
feedback counterexample in Supplementary Section~S6 likewise shows that a scheduling
advantage can disappear or reverse. Replaying into a
...
The first asks whether the uniform certificate is informative at the waits that
matter. Four of the original six workloads fail this usefulness screen: the first-come first-served p99 wait, in multiples of $L$, is \devnum{0.021} on ACcoding, \devnum{0.136} on the
compute farm, \devnum{0.373} on continuous-integration pool~B and \devnum{1.78} on
pool~A, against the additive term of about
\devnum{3}$L$ in the uniform certificate. This comparison does not establish
impossibility below that term for a restricted base policy or workload. On the Intel
...
window we build is \devnum{18.7} hours, so the uniform additive term is
about \devnum{17} days and the guard never fires at any of the positive budgets tested;
...
four workloads that fail this condition measure the ordering mechanism there; the
uniform bound is too coarse to
provide a useful promise at the reported tail scale.
The second is an empirical opportunity screen: concentrated service costs can
make reordering useful. A heavy tail is not a theorem assumption or a necessary
...
will be opened once, after the method, the parameters and the primary metric have
been frozen, and their numbers will be reported separately; that evaluation is not
completed in the results printed here. The second judging platform's sealed id block
and the corroborating trace's sealed year are not opened here: the frozen pipeline
covers the main trace alone, and opening them would need a second pipeline under the
same lock, which we did not build.
...
then re-impose inside the pool. The observed \devnum{60}-second regime is not documentation of an enforced timeout;
how the platform enforces such a limit is undocumented in the sources we found. We wrote to the
...
process is synthetic. The Firefox CI checks use recorded queue waits and fit an effective capacity and a setup term, both refitted
...
Original-clock scores may use outcomes that are still pending in the replay.
The conservative headline reduces history and changes fitted models as well as
release delay, so its difference from the original score does not identify a
queue-delay effect alone. The pending exact variant certifies availability for
its final same-copy replay, not an online fixed point or a deployment with globally
shared copied-user state (Section~\ref{sec:exp_visibility}).
...
\devnum{39\%} on its two physical partitions, with hourly correlations of
\devnum{0.44} and \devnum{0.40}. We therefore treat its policy replays as
conditional counterfactuals rather than validated estimates of deployment benefit.
Its archive describes serial-job FCFS ordering; queue concurrency ceilings and
middleware omitted by the replay prevent identifying its recorded waits with the
theoretical FCFS model. The work-budget guards never fire at the tested promises.
...
but we did not test one. Capacity is fixed within each replay; the capacity sweep is a sensitivity to that
choice, not an estimate of the original platform's capacity or a capacity-planning study. The reported runs compare policies at a common set of
...
The local timed-work service was executed, but there was no prospective production
deployment or measurement of user feedback. A deployed serverless scheduler of the kind this
