...
% Clean-score pair: the block headers of Table~\ref{tab:guard} (SPJF-E max excess
% 1,241.4 / 3,044.5 / 5,904.5 s) and its Guard(600) "Max exc." rows
% (492.2 / 507.8 / 523.7 s).  Corrupted pairs: Table~\ref{tab:adv}, columns
...
\pgfplotsset{
  excaxis/.style={
    width=5.3cm, height=4.6cm, ybar=1pt, bar width=4pt,
    symbolic x coords={clean,rev,rand,l-as-s}, xtick=data,
    ymin=0, ymax=6900, enlarge x limits=0.16,
...
\begin{axis}[excaxis, ylabel={worst excess over FCFS (s)},
  ...
  \node[anchor=south west, font=\scriptsize] at ({rel axis cs:0.01,0}|-{axis cs:clean,600}) {$G = 600$ s};
\end{axis}
\begin{axis}[excaxis, title={$\rho = 0.8$, $k = 5$}, yticklabels={},
             at={(q1.east)}, anchor=west, xshift=6mm, name=q2]
...
\begin{axis}[excaxis, title={$\rho = 1.0$, $k = 4$}, yticklabels={},
             at={(q2.east)}, anchor=west, xshift=6mm,
...
\end{axis}
\end{tikzpicture}}
\caption{The worst single job, with and without the guard, under the
expected-cost score (``clean'') and the three corrupted scores of
Table~\ref{tab:adv}: reversed (rev), random (rand) and truly-longest-called-shortest
...
Table~\ref{tab:guard}; the three corrupted pairs are the ``Max excess'' columns
of Table~\ref{tab:adv}. The unguarded bar grows with load and with corruption;
the guarded bar does not, which is the content of Theorem~\ref{thm:guard} and
