...
\begin{tikzpicture}[x=1cm,y=1cm,font=\footnotesize]
  ...
  % ---- row 2: shortest first  (vertical extent tightened, referee item G11)
  \node[lbl] at (-0.15,-1.25) {shortest first};
  \draw[->,gray] (0,-1.70) -- (8.6,-1.70);
  \node[jobbox,fill=black!25,minimum width=0.55cm,anchor=west] (b2) at (0,-1.25) {$B$};
  \node[jobbox,fill=black!25,minimum width=0.55cm,anchor=west] (c2) at (0.55,-1.25) {$C$};
  \node[jobbox,fill=black!8,minimum width=5.6cm,anchor=west] (a2) at (1.1,-1.25) {};
  \node at (3.9,-1.25) {$A$: 60 s};
  \draw[decorate,decoration={zigzag,segment length=3pt,amplitude=1.2pt}] (5.0,-1.55) -- (5.0,-0.95);
  \node[anchor=west] at (6.9,-1.25) {mean wait 0.2 s};

  % ---- waits
  \draw[<->] (0,-0.55) -- node[below,inner sep=2pt] {$B$ waits 60.0 s} (5.6,-0.55);
  \draw[<->] (0,-2.02) -- (1.1,-2.02);
  \node[anchor=west,inner sep=2pt] at (1.25,-2.02) {$A$ waits 0.4 s};
\end{tikzpicture}
...
waiting off $B$ and $C$ and puts \devnum{0.4} seconds onto $A$.}
\label{fig:threejob}
\end{figure}
