...
这个仓库放的是一篇论文背后的代码、导出表格和运行日志。论文题目是 *Prediction-Driven
Non-Preemptive Scheduling with Bounded Overtaking for Shared Execution Services under
Deadline-Driven Bursty Load*。

论文研究的是一类很多系统都有的队列。一个服务接收许多用户提交的任务，放到固定的几台机器上跑，
每台机器一次跑一个任务，任务一旦开始就不会被打断，只有跑到时间上限 `L` 才会被停掉。编程课的
自动评测机就是这样工作的。无服务器平台、软件构建集群和计算集群也是。

论文从一个具体的麻烦出发。作业截止前，几百个学生同时提交。一份死循环的提交会占住一台评测机
直到时间上限，排在它后面的每一份提交都得等。按预测的运行时长给队列重新排序，能让大多数提交
少等，但预测一旦出错，某一个任务就会被一次次挤到后面：在我们研究的这份日志里，有一个任务比
按到达顺序多等了将近 6,000 秒。没有哪个运营者敢把会这样做的调度器放上线。

论文给出的答案是对每个任务的一条承诺。它证明：在任何这样的 `k` 台机器的服务上，一个任务比按
到达顺序（先来先服务）多等的时间，等于插到它前面的工作量除以 `k`，误差不超过 `(2-2/k)L`。
然后给任意一种排序规则套一个守卫，限制允许插到一个等待任务前面的工作量，这样无论预测怎么错，
没有任务会比按到达顺序多等超过 `G` 秒。这个结论在两个编程课评测平台、一个无服务器平台、两个
持续集成池和一个计算集群的真实日志上做了检验。

这个仓库里是仿真器、预测器、实验驱动脚本，以及论文印出的每个数字背后的证据。它是复现包，不是
可以直接部署的调度器。
