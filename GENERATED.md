...
| `outputs/paper_tables/tab_*_sealed.tex`（冻结之后才存在） | sealed main、可选 k1、predictor 与 visibility CSV | `UV python scripts/emit_paper_tables.py --sealed-dir outputs/sealed_tables --sealed-predictor-dir outputs/sealed_predictor --sealed-visibility-dir outputs/sealed_visibility` | `check_paper_numbers.py` 带同三项；可见性表与 prose 数字同样按 `\sealednum{}` 和 CSV 来源检查 |
| `outputs/paper_tables/tab_*.tex`、`numbers.csv` | `outputs/dev_tables/` + `outputs/dev_visibility/` + `paper/` 里所有 `\devnum{}`（只读） | `UV python scripts/emit_paper_tables.py` | 原六表之外写 `tab_visibility.tex` 与 `tab_visibility_audit.tex`；数字索引认两份 visibility CSV。其余 location-agnostic 检查规则不变 |
