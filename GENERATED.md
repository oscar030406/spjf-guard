...
| `outputs/paper_tables/tab_*.tex`、`numbers.csv` | `outputs/dev_tables/` 的 CSV + `paper/` 里所有 `\devnum{}`（只读） | `UV python scripts/emit_paper_tables.py` | 两道：`UV python scripts/check_generated.py --only paper_numbers`（每个数要么指得出产它的表，要么写明本包为什么不产它）与 `UV python scripts/check_paper_numbers.py`（论文表体与本包表体逐值相同，正文与图注里的数字从 `outputs/dev_tables/*.csv` 重算得出，有来源的行仍然印着） |
